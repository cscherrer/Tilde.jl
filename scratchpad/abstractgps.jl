using AbstractGPs, KernelFunctions, Distributions, Tilde, LinearAlgebra, Plots, Random
...
pred = [posterior(p._y_dist, y) for p in predict(Tilde.withdistributions(m)(x=x), post)]
...
[posterior(p._y_dist, y) for p in predict(Tilde.withdistributions(m)(x=x), post)]