...
# We define this model using Tilde:
...
# In Tilde, models are _first-class_ and _function-like_, and applying a model to its arguments gives a _joint distribution_.

# Just a few of the things we can do in Tilde:
...
# Note that I said "turns into" instead of "interprets". Tilde uses [`GG.jl`](https://github.com/thautwarm/GG.jl) to generate specialized code for a given model, inference primitive (like `rand` and `logdensity`), and type of data.
