...
@inline function MeasureBase.logdensity_def(mc::ModelClosure, data; cfg = NamedTuple(), ctx=(ℓ=0.0,), call=nothing)
    cfg′ = merge(cfg, data)
    gg_call(mc, tilde_logdensity_def, cfg′, ctx)
end

@inline function tilde_logdensity_def(xname, l, d, cfg, ctx::NamedTuple, vars)
    ℓ = ctx.ℓ
    x = getproperty(cfg, xname)
    Δℓ = logdensity_def(d, l(x))
    ℓ += Δℓ
    @reset ctx.ℓ = ℓ
    (x, ctx, ℓ)
end
...
#             rhs = st.rhs
#             @q begin
#                 _ℓ += Tilde.logdensityof($rhs, $x)
#                 $x = Tilde.predict($rhs, $x)
#             end
#         end
...
@inline function tilde(::typeof(unsafe_logdensityof), lens, xname, x, d, cfg, ctx::NamedTuple)
    lm = lazymerge(cfg.args, cfg.obs, cfg.pars)
    x = getproperty(lm, dynamic(xname))
