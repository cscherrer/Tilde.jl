...
@inline function tilde(::typeof(weightedrand), x::Observed{X}, lens, d, cfg, ctx::NamedTuple) where {X}
    x = value(x)
...
@inline function tilde(::typeof(weightedrand), x::Unobserved{X}, lens, d, cfg, ctx::NamedTuple) where {X}
