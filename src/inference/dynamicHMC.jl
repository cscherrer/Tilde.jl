...
Draw `N` samples from the posterior distribution of parameters defined in Tilde model `m`, conditional on `_data`. Samples are drawn using Hamiltonial Monte Carlo (HMC) from the `DynamicHMC.jl` package.
