...
Draw `N` samples from the posterior distribution of parameters defined in Tilde model `m`, conditional on `data`. Samples are drawn using Hamiltonial Monte Carlo (HMC) from the `advancedHMC.jl` package.
